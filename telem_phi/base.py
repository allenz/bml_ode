...
# TODO refactor using classes (t.plot..)
...
def plotPAT(t):
    ...
    # Actual and commanded leg position
    plot.subplot(3,1,1)
    plot.plot(t.time,t.posR%(2.0*np.pi), 'k')
    plot.plot(t.time,t.posL%(2.0*np.pi), 'b')
    plot.plot(t.time,t.refR%(2.0*np.pi), 'k--')
    plot.plot(t.time,t.refL%(2.0*np.pi), 'b-.')
    tWidth(t, pad=500)
...
def plotForceLR(t):
    ...
    tWidth(t, ax)
    ax.legend(['Roll','Force RF','Force RM', 'Force RB'], loc=2)
    ...
    bx.legend(['Roll (R+)','Force LF','Force LM', 'Force LB'], loc=2)
